...
export default {
  ...
  methods: {
    ...
    onSubmit () {
      ...
      axios
        .post('http://80.240.23.127:8081/api/obnovenie-hesla/validaciaEmailu', this.emailJson)
        .then(response => {
          if (response.status === 200) {
            if (!response.data.valid) {
              ...
              axios
                .post('http://80.240.23.127:8081/api/obnovenie-hesla/odoslanieEmailu', this.emailJson)
                .then(response => {
                  if (response.status === 200) {
